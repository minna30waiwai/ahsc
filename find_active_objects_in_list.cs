...
public class find_active_objects_in_list : MonoBehaviour {
	...
	//return...
	void get_object_parameters(GameObject obj){
		...
		object_parameters_to_csv(para_table);

	}

	//
	void object_parameters_to_csv(IDictionary <string,string>para_table){
	...
	void csv_write(string write_data){	
		...
		try
		{
			...
			//出力
			using (var write_file=new System.IO.StreamWriter(csv_file_path+@"save_csv\test.csv", append,System.Text.Encoding.GetEncoding("utf-8")))
			{
